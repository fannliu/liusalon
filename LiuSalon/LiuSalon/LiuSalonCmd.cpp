...
MStatus LiuSalonCmd::doIt( const MArgList& args )
{
	...
	for(int i=0; i< hairCount; ++i){
		...
		//MString ns = num_of_segments + " ";
		int n = num_of_segments - 1;
		MString createCurve = MString("curve -d ") + n;
	...
	// group the nurbs curves
	MString groupCurves = MString("select -r ");
	for(int i=1; i <= hairCount; ++i){
		groupCurves = groupCurves + "curve" + i + " ";
	}

	MGlobal::executeCommand(groupCurves +"; group;");
