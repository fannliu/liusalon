	...
	uniform float azimuthalShiftG = 40;//Gangle????different per strand
{
    ...
    {
        float M_R = g(longitudinalWidthR, theta_h - longitudinalShiftR);
    ...
    {
        float M_TT = g(longitudinalWidthTT, theta_h - longitudinalShiftTT);
        float N_TT = g(azimuthalWidthTT, PI - phi);
    ...
    {
        float M_TRT = g(longitudinalWidthTRT, theta_h - longitudinalShiftTRT);
        
		float N_TRT_minus_G = cos(phi * 0.5);
        float N_G = intensityG * g(azimuthalWidthG, azimuthalShiftG - phi);
